public class Bird {
    ...
    public void rotation() {
        if (pipes.size() >= 1) {
            max_rotation_angle = 87f - (3 * pipes.get(0).getHorizontalVelocity());
        }
    ...
    public void score() {
        ...
        if (position.x >= score_node.x) {
            ui.score++;
            score_node.x += Pipe.distance_between_pipes;
        }
        score_node.x -= pipes.get(0).getHorizontalVelocity();
    }
